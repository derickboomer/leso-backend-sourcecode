...
app.post("/createaccount", (req, res) => {
  ...
  const newdate = moment(dateCreated).format("LLLL");

  const transporter = nodemailer.createTransport({
    service: "Gmail",
    auth: {
      user: "leso.capstone.test@gmail.com",
      pass: "oparaqqihuqljlqw",
    },
  });
  ...
  const options = {
    ...
    to: email,
    subject: "CAMRS LESO - Account Registration",
    text: `*This is a message from LESO Office* `,

    html: `<h3>Hi <strong>${firstName}</strong>,</h3>
     <p> This is to inform you that as of <strong> ${newdate}</strong> you are officially registered as <strong>${accountType}</strong> in the LESO: Care and Maintenance Job Requisition System </p>.
     <p> For more inquiries please email LESO.</p>  
...
app.get("/auditlogs", (req, res) => {
  con.query(
    "SELECT * FROM audit_logs ORDER BY idjob_order DESC ",
    (err, result) => {
      if (err) {
        console.log("Database error");
      } else {
...
app.put("/openjoborderupdate/:idjob_order", (req, res) => {
  ...
  const options = {
    ...
    text: `*This is a message from LESO Office* `,
    html: `<p>Hi <strong>${jobordervalues[5]}</strong>,</p>
    <p>This is to inform you that your <strong>Job Order Request:${idjob_order}</strong> has been closed on <strong> ${jobordervalues[23]}</strong>.</p> 
    <p>You may now view or print your "Technical Service Report" and "Certificate of Calibration"  on the Track Tab by placing your Tracking ID: <strong>${jobordervalues[30]}</strong>.</p>
    <p>You may also visit <a href=" http://localhost:3000/react-gh-pages?idjob_order=${jobordervalues[30]}#/Client/Track/joborderupdate/${idjob_order}">this link</a> to get an official copy of the certificate with the administrator's signature.</p>
    <strong><p>For more questions please email __________ Thank You!</p></strong>
      `,
  };

  const sqlUpdate =
    "UPDATE job_order SET `trackingnumber` = ?, `idjob_order` = ?, `type` = ?, `status` = ?, `stage` = ?, `firstName` = ?, `lastName`= ?, `email` = ?, `cellphoneNo` = ?, `image` = ?, `quantity` = ?, `serviceRequested` = ?, `instrument` = ?, `college` = ?, `laboratory` = ?, `laboratoryLocation` = ?, `problemReported` = ?, `model` = ?, `serialNo` = ?, `manufacturer` = ?, `propertyNo` = ?, `diagnosis` = ?, `actionTaken` = ?, `dateCalibrated` = ?, `procedure` = ?, `calibrationResult` = ?, `remarks` = ? , `approvedBy` = ? , `receivedby` = ? , `recommendation` = ?, `trackingnumber` = ? , `auditlog` = ?  WHERE idjob_order = ?";
  con.query(sqlUpdate, [...jobordervalues, idjob_order], (err, result) => {
    console.log(jobordervalues);
    if (err) {
      console.log(err);
    } else if (jobordervalues[2] == "Closed") {
      res.send(result);
  ...
  // req.body.trackingnumber, 0
  // req.body.idjob_order, 1
  // req.body.type, 2
  // req.body.status, 3
  // req.body.stage, 4
  ...
  // req.body.lastName, 6
  // req.body.email, 7
  // req.body.cellphoneNo, 8
  // req.body.image, 9
  // req.body.quantity, 10
  // req.body.serviceRequested, 11
  // req.body.instrument, 12
  // req.body.college, 13
  // req.body.laboratory, 14
  // req.body.laboratoryLocation, 15
  // req.body.problemReported, 16
  // req.body.model, 17
  // req.body.serialNo, 18
  // req.body.manufacturer, 19
  // req.body.propertyNo, 20
  // req.body.diagnosis, 21
  // req.body.actionTaken, 22
  // req.body.dateCalibrated, 23
  // req.body.procedure, 24
  // req.body.calibrationResult, 25
  // req.body.remarks, 26
  // req.body.approvedBy, 27
  // req.body.receivedby, 28
  // req.body.recommendation, 29
  // req.body.trackingnumber,30
  // new Date(), 31

  const sqlInsert =
    "INSERT INTO audit_logs (trackingnumber, type, status, stage, firstName, lastName, serviceRequested, instrument, college, recommendation, auditlog, approvedBy, receivedby) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
  con.query(
    sqlInsert,
    [
      jobordervalues[0],
    ...
    (err, res) => {
      if (err) {
        ...
        console.log("Inserted Audit Log Table");
      }
    }
  );
});
...
app.post("/auditlogsfilter", (req, res) => {
  const trackingnumber = req.body.trackingnumber;

  const sqlLogin =
    "SELECT * FROM audit_logs WHERE trackingnumber = ? ORDER BY idjob_order DESC";
  con.query(sqlLogin, [trackingnumber], (err, result) => {
    if (err) {
      console.log(err);
